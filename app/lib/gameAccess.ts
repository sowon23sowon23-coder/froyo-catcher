...
export function normalizeGameAccessConfig(input: unknown): GameAccessConfig {
  ...
  const blockMessage = typeof raw.blockMessage === "string" && raw.blockMessage.trim()
    ? raw.blockMessage.trim().slice(0, BLOCK_MESSAGE_MAX)
...
export function resolveGameAccessState(configInput: unknown, now = new Date()): GameAccessState {
  ...
  if (pageBlocked) {
    return {
      ...
      isOpen: false,
      reason: "blocked",
      message: config.blockMessage || DEFAULT_BLOCKED_MESSAGE,
      startsAt,
      endsAt,
      blocksAt,
      pageBlocked,
