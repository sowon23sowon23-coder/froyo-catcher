...
export async function POST(req: NextRequest) {
  ...
  const buildOkResponse = (
    entryId: number,
    sessionContactType: EntryContactType = contactType,
    sessionContactValue: string = normalizedContact
  ) => {
    const res = NextResponse.json({
      ok: true,
      contactType: sessionContactType,
      contactValue: sessionContactValue,
    });
  ...
  if (nicknameOwner.data?.id) {
    ...
    if (updateResult.error) {
      return NextResponse.json({ error: updateResult.error.message || "Failed to update entry." }, { status: 500 });
    }
